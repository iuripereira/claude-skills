...
- Skill `sdd-iuri:handoff`: fecha a sessão nos registros com dono — atualiza o `STATE.md`
  ...
  a delta em curso com fase e veredito do gate. Própria, inspirada na `handoff` de
  mattpocock/skills (MIT), reescrita para gravar no repo em vez de brief efêmero. (delta-008)
...
- Notação viva das deltas passa de `ΔNNN` para `delta-NNN` (digitável) em templates, docs do
  ...
  imutável (ADRs, `_archive/`, changelog lançado) preserva o `Δ`. (delta-006, #17)
...
- `adapters.md` declara o fallback do review estágio 1 (conferência inline com aviso) —
  ...
  corrigidas na consolidação. (Δ005, #15)
