...
### Adicionado
- `doc-entregavel`: **Índice automático** em página própria após a capa, nos dois formatos — no pdf gerado dos títulos h1–h3 (extensão `toc` do python-markdown, links internos; sem nº de página, limitação do print do Chrome) e no docx como campo `TOC` nativo com `updateFields` (o Word preenche/atualiza ao abrir). Selftest cobre a presença do campo.
